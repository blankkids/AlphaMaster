...
class TestKeyNumbers:
    def test_feature_count(self):
        """特征数应 == 71（8 大类全覆盖 + TA-Lib 互补，R1.1）。"""
        assert len(FEATURE_NAMES) == 71, (
            f"期望 71 个特征，实际 {len(FEATURE_NAMES)}"
        )
    ...
    def test_vocab_size(self):
        """词表总大小 == 71 + 62 == 133。"""
        assert FORMULA_VOCAB.size == 133, (
            f"期望 vocab size=133，实际 {FORMULA_VOCAB.size}"
        )

    def test_feat_offset(self):
        """feat_offset == 71（feature token id ∈ [0,70]，operator id ∈ [71,132]）。"""
        assert FORMULA_VOCAB.operator_offset == 71
...
class TestComputeFeatures:
    def test_shape(self):
        ...
        assert feats.shape == (3, 71, 200), (
            f"features 形状期望 [3,71,200]，实际 {tuple(feats.shape)}"
        )
...
class TestStackVM:
    ...
    def test_feat_offset_is_71(self):
        vm = StackVM()
        assert vm.feat_offset == 71
...
def test_full_pipeline_smoke(tmp_path):
    ...
    # 2. compute_features → [3, 71, 200]
    feats = MT5FeatureEngineer.compute_features(raw)
    assert feats.shape == (3, 71, 200), f"特征形状错误: {tuple(feats.shape)}"
